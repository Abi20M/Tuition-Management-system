...
const Month = React.forwardRef((props, ref) => {
  ...
  const { classes, cx, theme } = Month_styles['default']({ fullWidth }, { classNames, styles, unstyled, name: __staticSelector });
  ...
  const firstIncludedDay = React.useMemo(() => days.flatMap((_) => _).find((date) => {
  ...
  }) || dayjs__default(month).startOf("month").toDate(), []);
  const rows = days.map((row, rowIndex) => {
    const cells = row.map((date, cellIndex) => {
      ...
      return /* @__PURE__ */ React__default.createElement("td", {
        ...
        firstInMonth: isSameDate.isSameDate(date, firstIncludedDay),
