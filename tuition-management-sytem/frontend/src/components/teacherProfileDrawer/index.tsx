import {
  ...
  export const TeacherDrawer = (props: iProps) => {
    ...
    return (
      <Container>
        ...
        <h3 style={{ textAlign: "center" }}>{props.teacherDetails.name}</h3>
        <h6
          style={{ textAlign: "center", color: "GrayText", marginTop: "-10px" }}
        >
          <i>{props.teacherDetails.email}</i>
        </h6>
        {/* input fields of user data */}
        <Stack p={20} mt={-10}>
          <TextInput
            label="Name"
            value={props.teacherDetails.name}
            contentEditable={false}
            readOnly
          />
          <TextInput
            label="Email"
            value={props.teacherDetails.email}
            contentEditable={false}
            readOnly
          />
          {/* <TextInput
